...
module.exports = (app) => {
  ...
  const topGainers = (list) => {
    ...
    const copy = list.slice(0, 5)

    return copy.map(item => ({
      name: item,
      value: prices[item].CHANGEPCT24HOUR,
    }))
  }
  ...
  module.setup = async () => {
    ...
    setInterval(async () => {
      ...
      if (chunkCounter + chunkSize < chunk.length) {
        ...
        // Provide a default sort all coin prices obtained
        sortByMktCap(app.locals.coinList)
        sortByMktCap(app.locals.watchList)

        const coinIds = app.locals.coinList.map(item => item.id)
        app.locals.topGainers = topGainers(coinIds)
        app.locals.topLosers = topLosers(coinIds)
